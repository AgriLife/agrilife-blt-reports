...
class Templates_EducatorDocs {
	...
	public function get_archive_template( $archive_template ) {
		...
		if ( is_post_type_archive( 'educator-documents' ) ) {
			$archive_template = AG_BLT_TEMPLATE_PATH . '/archive-educatordoc.php';
		}
	...
	public function get_single_template( $single_template ) {
		...
		if ( get_query_var( 'post_type' ) == 'educator-documents' ) {
			$single_template = AG_BLT_TEMPLATE_PATH . '/single-educatordoc.php';
		}
